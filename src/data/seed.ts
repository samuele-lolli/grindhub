...
export function seedData() {
  ...
  mockPlayers.slice(0, 10).forEach((player, i) => {
    const postTypes: Array<{ type: SocialPost['type']; content: string }> = [
      { type: 'session_result', content: `🏆 Just shipped a $${Math.floor(Math.random() * 5000 + 500)} score in the ${['Sunday Million', 'Bounty Builder', 'GGMasters', 'Daily Grind'][Math.floor(Math.random() * 4)]}!` },
      { type: 'milestone', content: `📈 Reached $${Math.floor(Math.random() * 10000 + 1000)} total profit! The grind pays off 💪` },
      { type: 'text', content: ['Studying ICM spots all day, this game never stops evolving 📚', 'Just moved up to $55 MTTs, wish me luck! 🚀', 'Back to back final tables, feeling great about my game 🔥', 'Taking a break today, mental game is just as important 🧘'][Math.floor(Math.random() * 4)] },
      { type: 'goal_completed', content: `🎯 Goal completed: Played 100 tournaments this month! Consistency is key.` },
    ];
    ...
    posts.push({
      ...
      comments: Math.random() < 0.4 ? [{
        ...
        content: ['Nice! 🔥', 'GG!', 'Keep grinding! 💪', 'Impressive run!', 'Well played'][Math.floor(Math.random() * 5)],
        createdAt: new Date(now - Math.random() * 2 * 24 * 60 * 60 * 1000).toISOString(),
      }] : [],
      createdAt: new Date(now - i * 8 * 60 * 60 * 1000 - Math.random() * 4 * 60 * 60 * 1000).toISOString(),
      isPublic: true,
    });
  });
  ...
  localStorage.setItem('grindhub-bankroll', JSON.stringify({ state: { accounts, transactions }, version: 0 }));
  localStorage.setItem('grindhub-profile', JSON.stringify({ state: { profile: currentUser, players: mockPlayers }, version: 0 }));
  localStorage.setItem('grindhub-social', JSON.stringify({ state: { posts, following }, version: 0 }));
  localStorage.setItem('grindhub-goals', JSON.stringify({ state: { goals, achievements: [] }, version: 0 }));
  localStorage.setItem('grindhub-settings', JSON.stringify({
    state: { settings: { currency: 'EUR', locale: 'en', theme: 'dark', defaultGameType: 'mtt', defaultPlatform: 'pokerstars', autoShareSessions: false } },
    version: 0,
  }));
